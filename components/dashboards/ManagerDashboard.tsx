...
export default function ManagerDashboard({ user, setUser }: { user: any, setUser: any }) {
  const [tickets, setTickets] = useState(seed),
    ...
    [view, setView] = useState<"Board" | "List" | "Reports" | "Users" | "Settings">("Board"),
  ...
  const myInitials = useMemo(() => teamMembers.find(m => m.id === user?.id)?.initials || "AK", [teamMembers, user]);
  const myTicketCount = useMemo(() => tickets.filter(t => t.who === myInitials).length, [tickets, myInitials]);
  const shown = useMemo(
    () =>
      tickets.filter(
        (t) =>
    ...
    <main>
      <aside>
        ...
        <div className="aside-bottom">
          ...
          <a onClick={() => { setView("Settings"); setPanel(null); void loadAdmin(); }}>⚙ Settings</a>
          <div className="profile">
            ...
            <span>
              <b>{user.name}</b>
              <small>{user?.role ? user.role.replace("_", " ") : "User"}</small>
            </span>
          </div>
        </div>
      </aside>
      <section className="workspace">
        <header>
          ...
          <div className="account">
            <button className="icon-btn" onClick={() => setPanel("help")} title="Help">
            ...
            <button className="icon-btn" onClick={() => setPanel("notifications")} title="Notifications">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="18" height="18">
        ...
        <div className="content">
          ...
          <div className="tabs">
            ...
            <span />
            <button className={view === "Settings" ? "on" : ""} onClick={() => { setView("Settings"); void loadAdmin(); }}>
              ⚙ Project settings
            </button>
          </div>
          {view !== "Users" && view !== "Settings" && <div className="tools">
            <div>
              <select
                      ...
                      .map((t) => (
                        ...
                        >
                          ...
                          <footer>
                            ...
                            <span onClick={(e) => e.stopPropagation()} title="Reassign ticket" style={{ position: 'relative', display: 'inline-block', width: '29px', height: '29px' }}>
                              <i className="avatar" style={{ background: teamMembers.find((m)=>m.initials===t.who)?.color || people[t.who]?.[1] || '#ccc' }}>
                                {t.who}
                              </i>
                              <select 
                                value={teamMembers.find((m)=>m.initials===t.who)?.id??""} 
                                onChange={(e)=>void changeAssignee(t.id,Number(e.target.value))}
                                style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', opacity: 0, cursor: 'pointer' }}
                              >
                                {teamMembers.map(m => <option key={m.id} value={m.id}>{m.initials}</option>)}
                              </select>
                            </span>
                          </footer>
                        </article>
                ...
                >
                  ...
                  <span onClick={(e) => e.stopPropagation()}>
                    <select 
                      value={teamMembers.find((m)=>m.initials===t.who)?.id??""} 
                      onChange={(e)=>void changeAssignee(t.id,Number(e.target.value))}
                      style={{ border: '1px solid #ddd', borderRadius: '4px', padding: '4px 8px', background: 'transparent', cursor: 'pointer' }}
                    >
